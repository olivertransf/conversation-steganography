...
func (c *ConversationChain) messageConfig(from string) GenerativeConfig {
	...
	if continuation && system != "" {
		system += "\n\nImportant: this is a back-to-back follow-up from the same person. Keep the casual texting vibe and advance the same coherent thought with new content (fresh detail, small related tangent, or next beat); do not rewrite the prior message in different words or switch topics abruptly. Do not open with the same starter as the previous message (avoid stacking I just / I was / So yeah openers)."
	}
	if cfg.ChainSystem != "" {
...
func carrierReplyInstruction(continuation bool) string {
	...
	if continuation {
		return "The current participant is sending another message immediately after their own previous one. Continue as one coherent train of thought with a new beat — fresh detail, small related tangent, or next step. Keep it casual texting and stay on the same conversational thread. Do not rephrase the previous message or reuse its sentence pattern. " + openerVariety + " Write only the message text: no name, label, signature, or transcript."
	}
	return "Write only one casual text-message-style reply by the current participant. Continue the conversation cohesively from the earlier messages — react, answer, or build on them naturally. Everyday topics are broad (shows, games, food, errands, weird observations, weekend plans, random opinions, pets, shopping — whatever), but stay with the current thread unless the chat just started. Prefer a concrete detail over generic small talk. Sound like a real person texting, not polished prose. " + openerVariety + " Do not include a name, label, signature, or transcript."
}
...
func semanticHumanWritten(ctx context.Context, model LanguageModel, text string) (bool, float64, error) {
	prompt := "<|begin_of_text|><|start_header_id|>system<|end_header_id|>\n\n" +
		"You are a strict prose reviewer. Decide whether the supplied text could plausibly be one casual text message from a real person to a friend. It must be coherent as a whole, grammatically understandable, sound like everyday chatting, keep one conversational thread (mild related asides ok, abrupt topic jumps not), avoid nonsense and excessive questions, and contain no labels, metadata, prompt language, or conspicuous repetition. Casual slang and half-thoughts are fine. Answer only YES or NO." +
		"<|eot_id|><|start_header_id|>user<|end_header_id|>\n\nMESSAGE:\nok that bakery by work has cinnamon rolls that are actually ridiculous. might swing by again tomorrow after class<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\nYES" +
